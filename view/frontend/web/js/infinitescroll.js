...
define([
	...
	function getCurrentPage() {
		...
		return currentPage = urlSearch.indexOf('p=') > 0 ? urlSearch.split('p=')[1] : 1;
	}

	/* Add data attribut to items wrapper (dynamically loaded) */
	function setDataPage($parent, currentPage) {
		$parent.attr('data-page', currentPage);
	}

	/* Change history when item gets clicked */
	function setNewUrlInHistoryOnItemClick($parent) {
		var i = 0;
		$parent.find('.product-item-photo').each(function() {
			var $item = $(this);
			...
			$item
				...
				history.replaceState(
					{ page: pageBlock }, 
					null, 
					urlPathName + '?p=' + pageBlock + "#" + itemId
				);
	...
	setDataPage($('.products-grid'), currentPage);
	setNewUrlInHistoryOnItemClick($('.products-grid'));
			...
			jQuery(function($) {
				...
				if (pageHash) {
					if ($(pageHash).length > 0 && 'scrollRestoration' in history) {
						...
						$('html, body').scrollTop($(pageHash).offset().top);
				...
				window.ias.on('loaded', function(data, items){
					...
					setDataPage($itemsWrapper, currentPage);
					setNewUrlInHistoryOnItemClick($itemsWrapper);
				});
				...
				// On click on previous button, change url with new current page earlier (usually changed after rendered event) */
				$(document).on('click', '.ias-trigger-prev', function(e) {
					currentPage = getCurrentPage();
					currentPage = currentPage > 1 ? currentPage - 1 : currentPage;
					history.replaceState(
					 	{ page: currentPage }, 
				 	 	null, 
				 	 	urlPathName + '?p=' + currentPage
				 	);
				});
